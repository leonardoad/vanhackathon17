...
class Db_Table extends Zend_Db_Table {
     ...
     * @throws Zend_Db_Table_Exception
     */
    public static function getOptionList2($keyName, $valueName, $orderName, $class, $firstEmpty = true, $readMethod = 'readLst') {
        if ($firstEmpty) {
//            $return[] = array('key' => '', 'value' => '---');
            $return[] = '---';
        }
    ...
    public function where($campo, $valor, $oper = '=', $glue = 'and') {
        if ($campo != '') {
            if (strpos(strtoupper($valor), 'NULL') !== FALSE) {
                $this->_whereSelect[] = array('campo' => "$campo $valor ", 'glue' => $glue);
            } elseif ($oper == 'like' || $oper == 'ilike') {
                $this->_whereSelect[] = array('campo' => "UPPER($campo) " . $oper . ' ? ', 'valor' => '%' . strtoupper(Format_String::htmlToString($valor)) . '%', 'glue' => $glue);
            } else {
